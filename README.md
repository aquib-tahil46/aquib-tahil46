# Hi, I'm Aquib Tahil 👋

### Data Analyst | Tableau Developer | BFSI Domain | 9+ Years

I turn complex banking and financial data into clear, decision-ready insights — through intuitive Tableau dashboards, robust SQL pipelines, and structured data storytelling.

Based in **Kolkata, India** · Open to **Remote & On-site** opportunities

[![LinkedIn](https://img.shields.io/badge/LinkedIn-aquibtahil-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/aquibtahil/)
[![Email](https://img.shields.io/badge/Email-a.tahil2@gmail.com-D14836?style=flat&logo=gmail&logoColor=white)](mailto:a.tahil2@gmail.com)

---

## 🛠 Tech Stack & Skills

**Visualisation**
`Tableau Desktop` `Tableau Prep` `Calculated Fields` `LOD Expressions` `Dual-Axis Charts` `Drill-Through` `Story Points`

**Data & Querying**
`SQL` `Joins & Subqueries` `Window Functions` `CTEs` `Query Optimisation` `Data Cleaning`

**Reporting**
`Advanced Excel` `Pivot Tables` `Power Query` `MIS Reporting` `VLOOKUP / INDEX-MATCH`

**Domain**
`Retail Banking` `Wealth Management` `Fraud Detection` `AML / KYC` `Transaction Analytics` `Finacle`

---

## 📂 Featured Projects

### 🏦 [Banking Transactions Analysis Dashboard](https://github.com/aquib-tahil46/banking-transactions-analysis)
> Tableau · MySQL

Interactive Tableau dashboard analysing banking transaction data — uncovering digital payment patterns, channel-wise trends, and customer segment behaviour across large transaction volumes.

---
🛒 E-Commerce Sales Dashboard

Power BI · Power Query · DAX · CSV

Interactive Power BI dashboard built from raw CSV data to analyze e-commerce sales, profit, quantity, payment modes, and state-wise performance.

Key insights include ₹438K sales, ₹37K profit, 5,615 units sold, ₹121K AOV, with Maharashtra as the top-performing state, Clothing contributing the highest quantity share, and COD as the most preferred payment mode.

The project demonstrates skills in Power BI dashboarding, DAX measures, Power Query data cleaning, sales analytics, and business insight generation.
---

### 🔍 [Fraud Detection & Risk Analysis Dashboard](https://github.com/aquib-tahil46/fraud-detection-tableau-dashboard)
> Tableau · SQL · Excel

Comprehensive fraud investigation dashboard built on 50,000+ transaction records. Features drill-through views by risk type, geography, amount band, and time of day. Rule-based logic reduced false-positive alerts by 30%.

---
### [🏦 BFSI Analytics Dashboard — Power BI](https://github.com/aquib-tahil46/BFSI-PowerBI-dashboard)

| Tableau · MySQL · DAX · Star Schema

End-to-end BI dashboard covering 7 BFSI business scenarios — Sales Trend, Revenue Decline, 
Loan Default, Customer Churn, Branch KPI, Region Performance, and Product Profitability. 
Built on MySQL star schema with 6 SQL views and 12 DAX measures.

**Key Finding:** Bank is aggressively selling Loans (24% margin, 50% default rate) while 
Deposits and Insurance — 2× more profitable — sit underutilised. Data makes the case for 
a product mix rebalancing.

---
### 🏦 [Loan Portfolio Risk Analysis Dashboard](https://github.com/aquib-tahil46/Loan-Portfolio-Risk-Analysis-Lending-Club-2007-2013-)
| Tableau · MySQL · Star Schema

End-to-end credit risk analytics on 100,000 Lending Club 
loans (2007–2013). 3-table star schema in MySQL with complex 
JOINs, CTEs, and Window Functions. Tableau dashboards using 
FIXED/INCLUDE/EXCLUDE LOD, Dual Axis, 8 chart types, 
2 dashboards + 2 story points.

**Key Finding:** Grade G — 27% NPA with <3% recovery rate.

[📊 Live Dashboard](https://public.tableau.com/app/profile/aquib.tahil8642/viz/Lending_club_17775741205830/Dashboard1?publish=yes) | [💻 GitHub Repo](https://github.com/aquib-tahil46/Loan-Portfolio-Risk-Analysis-Lending-Club-2007-2013-)

---
### 🏦 [Wealth Banking MIS Migration Dashboard](https://github.com/aquib-tahil46/wealth-mis-migration-dashboard)
| Tableau · MySQL · Executive BI Story

End-to-end Wealth Banking MIS migration from Excel to MySQL + Tableau. Covers ₹21.54B portfolio across 5 zones, 11 products, and 100 Wealth Managers. Full SQL pipeline — 4-step data cleaning, 13 analytics queries, CTEs, Window Functions, and RANK(). Two executive dashboards + 7-point Tableau Story with KPI tiles, bullet charts, heatmap, bubble chart, treemap, and performance segmentation.

**Key Finding:** Greater Kolkata trails Kolkata South by 15.4% — HL & Deposits carry ₹0.93B gap each, with 5 Critical WMs requiring immediate intervention.

[📊 Live Dashboard](https://public.tableau.com/app/profile/aquib.tahil8642/viz/Wealth_MIS_Migration_Executive_Dashboard/Story1) | [💻 GitHub Repo](https://github.com/aquib-tahil46/wealth-mis-migration-dashboard)
---
### 🌐 Cross-Border Payment Flow Visualisation
> Tableau · XRP · R3 Corda

Stakeholder-ready dashboards visualising end-to-end cross-border payment flows — highlighting settlement timelines, cost reduction opportunities, and compliance checkpoints across XRP and R3 Corda architecture.

---

## 🎓 Certifications

| Certification | Issuer | Issued |
|---|---|---|
| ✅ **Salesforce Certified: Tableau Desktop Foundations** | Salesforce / Trailhead | April 2026 |
| 📜 Professional Certificate in Blockchain | IIT Kanpur | Jan 2026 |
| 📜 Professional Certificate in FinTech | IIT Delhi | Apr 2025 |
| 📜 Microsoft Excel: Beginner to Expert | Udemy | Apr 2025  |

> 🔗 Tableau cert Credential ID: **7616752** · [Verify here](https://sforce.co/verifycerts)

---

## 💼 Experience Snapshot

| Role | Organisation | Period |
|---|---|---|
| Reporting Analyst – Wealth Management | ICICI Bank | Apr 2023 – Jan 2026 |
| Customer Analytics & Service Manager | Axis Bank | Jun 2016 – Jan 2023 |

---

## 🎓 Education

- **MBA – Finance** · National Institute of Technology (NIT), Durgapur · 2014–2016
- **BBA – Finance** · Burdwan Raj College · 2011–2014

---

*Always open to connecting with fellow data professionals, BI practitioners, and BFSI domain experts.*
